...
const subdomains = [
  ...
  'api.cobalt.meowing.de',
  'api.cobalt.eversiege.network',
  'api.cobalt.xenon.zone',
  'api.cobalt.cjs.nz'
];
...
async function test() {
  ...
  for (const domain of subdomains) {
    console.log(`\nTesting Cobalt API subdomain: ${domain} ...`);
    try {
      const ip = await resolveDoh(domain);
      console.log(`  IP resolved: ${ip}`);
      
      const result = await cobaltPost(ip, domain, '/api/json', postData);
      console.log(`  Status: ${result.status}`);
      console.log(`  Body:`, typeof result.body === 'object' ? JSON.stringify(result.body).substring(0, 300) : result.body.substring(0, 300));
      
      if (result.status === 200 && result.body && (result.body.status === 'redirect' || result.body.status === 'stream') && result.body.url) {
        console.log(`  ✅ SUCCESS! Resolved stream URL:`, result.body.url.substring(0, 100));
        return;
      }
    } catch (err) {
